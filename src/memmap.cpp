...
SCHERZO_INLINE uint32 caCRC32(uint8 *array, uint32 size, register uint32 crc32 = 0xFFFFFFFF);
...
again:
	...
	else CheckForIPSPatch (filename, HeaderCount != 0, TotalFileSize);
...
//MK: If the hack stopping Interleave 2 on Yoshi's is gone, we don't need this, either!
/*
#ifdef LSB_FIRST 
...
SCHERZO_INLINE uint32 caCRC32(uint8 *array, uint32 size, register uint32 crc32)
{
...
#define RomPatch(adr,ov,nv) \
	...
	//cmp on $213D
	//(Loads $2137, $213D, $213d, CMP)
    // Tetsuwan Atom
	//anomie recommendation...
...
//possibly fixed. Needs reverification!
//let's try removing this one. It works for MK
/*
    // Oda Nobunaga
