                         ...
                         S9xCheatDataSize size, bool8 is_signed, bool8 update)
{
    ...
    if (is_signed)
    {
        for (i = 0; i < 0x20000 - l; i++)
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->RAM, i), _DSx(size, d->CWRAM, i)))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        for (i = 0; i < 0x10000 - l; i++)
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->SRAM, i), _DSx(size, d->CSRAM, i)))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        for (i = 0; i < 0x2000 - l; i++)
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->FillRAM + 0x3000, i), _DSx(size, d->CIRAM, i)))
            {
                if (update)
    ...
    else
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->RAM, i), _Dx(size, d->CWRAM, i)))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->SRAM, i), _Dx(size, d->CSRAM, i)))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->FillRAM + 0x3000, i), _Dx(size, d->CIRAM, i)))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
                        ...
                        bool8 is_signed, bool8 update)
{
    ...
    if (is_signed)
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->RAM, i), (int32) value))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->SRAM, i), (int32) value))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _Cx(cmp, _DSx(size, d->FillRAM + 0x3000, i), (int32) value))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
    ...
    else
    {
        ...
        {
            if (TEST_BIT (d->WRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->RAM, i), value))
            {
                if (update)
                    d->CWRAM [i] = d->RAM [i];
            }
        ...
        {
            if (TEST_BIT (d->SRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->SRAM, i), value))
            {
                if (update)
                    d->CSRAM [i] = d->SRAM [i];
            }
        ...
        {
            if (TEST_BIT (d->IRAM_BITS, i) &&
                _Cx(cmp, _Dx(size, d->FillRAM + 0x3000, i), value))
            {
                if (update)
                    d->CIRAM [i] = d->FillRAM [i + 0x3000];
            }
