/* KallistiOS 1.2.0
   ...
   modified by scherzo to change sound buffer size
*/
...
void scherzo_snd_stream_set_callback(void *(*func)(int, int*)) {
	str_get_data = func;
}
...
static void scherzo_process_filters(void **buffer, int *samplecnt) {
	filter_t * f;
...
void scherzo_snd_stream_prefill() {
	...
	scherzo_process_filters(&buf, &got);
	scherzo_sep_data(buf, (BUFFER_SIZE/2));
...
int scherzo_snd_stream_init(void* (*callback)(int, int *)) {
	...
	/* Setup the callback */
	scherzo_snd_stream_set_callback(callback);
...
int scherzo_snd_stream_poll() {
	...
	if (needed_samples > 0) {
		if (stereo) {
			...
			scherzo_process_filters(&data, &got_samples);
